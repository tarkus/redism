...
Redism = (function() {
  Redism.prototype._ready = false;

  function Redism(options) {
    var client, clients, connected, name, scope, servers, total_clients, _i, _len, _ref, _servers;
    ...
    if (typeof this.options.servers[0] === 'string') {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (Array.isArray(scope[1])) {
          name = scope[0], servers = scope[1];
          this.servers.scopes[name] = servers;
        } else {
          servers = scope;
    ...
    client = _servers.forEach((function(_this) {
      return function(server) {
        var authparts, db, host, pass, port, serverparts, _ref1;
        serverparts = url.parse(server);
        if (serverparts.protocol !== 'redis:') {
          return console.error("Please use redis url instead " + server);
        }
        host = serverparts.hostname;
        port = parseInt(serverparts.port) || '6379';
        db = (_ref1 = serverparts.pathname) != null ? _ref1.slice(1 || null) : void 0;
        pass = null;
        if (serverparts.auth) {
        ...
        if (_this.server_list[server] == null) {
          _this.server_list[server] = 0;
        }
        _this.server_list[server] += 1;
        _this.client_list.push(server);
        if (_this.options.name) {
          ...
          client.on('connect', function() {
            ...
            if (clients === total_clients) {
              assert(connected === total_clients, "" + name + ": failed to connect some nodes, expected: " + total_clients + ", connected: " + connected);
              _this._ready = true;
              return console.log("" + name + ": " + connected + " nodes connected");
            }
          });
        }
        return client;
